...
class Activity:
	def __init__(self, activity_id, athlete_id):
		...
		self.cog = -1
		try:
			headers = {'Authorization': 'Bearer ' + get_access_token(athlete_id)}
	...
	def get_stream(self):
		...
		try:
			...
			headers = {'Authorization': 'Bearer ' + get_access_token(athlete_id)}
			params = {'keys': 'time,latlng,distance,altitude,heartrate,cadence,watts,temp,moving', 'key_by_type': 'true'}
			# TODO 
	...
	def generate_stream(self):
		...
		try:
			...
			stream['cadence'] = {
				...
				'series_type': 'time',
				'original_size': stream['distance']['original_size'], # do we need to verify that distance exists in this object?? probably should?
	...
	def requires_cadence_data(self):
		...
		try:
			...
			logging.error('error determining if this activity requires cadence data generated:')
			logging.error(e)
	...
	def replace_activity(self):
		...
		try:
			...
			headers = {'Authorization': 'Bearer ' + get_access_token(self.obj['athelte']['id'])}
			params = {
				'activity_type': self.obj['type'],
				'name': self.obj['name'],
				'description': self.obj['description'],# do we want to add a watermark? - similar to klimat?
				'trainer': 1 if self.obj['trainer'] else 0,
				'commute': 1 if self.obj['commute'] else 0,
				'data_type': create_gpx(self.generate_stream()) # we might want to use FIT instead since we can preserve more data across replacing (lap & session data +), but lower priority
