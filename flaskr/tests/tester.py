...
#     distances.sort()
#     max_cutoff = distances[int(len(distances) * .96)] * 60 / (
#         math.pi * (wheel_diameter + (2 * tire_width)) * (chainring / cog))
...
#                 cadence = float(generateCadence(
#                     (last_lat, last_lon), (lat, lon), cog, chainring, tire_width, wheel_diameter))
#                 m = mean(window)
#                 if (cadence > max_cutoff):
#                     cadence = 0
#                     # cadence = m
#                 elif (cadence > m + delta and cadence < m - delta):
#                     print('correcting bad value: ' + str(cadence) +
#                           "to this better value: " + str(m))
#                     cadence = m
#                 else:
#                     if (cadence > m):
...
#                 lon = float(element.attrib["lon"])
#             if (element.tag == elements['extensions']):
#                 distances.append(float(generateDistance(
...
#     derived_cadence = derivedCadenceList(
#         tree, cog, chainring, tire_width, wheel_diameter)
...
#     fig.add_trace(go.Scatter(x=seconds, y=actual_cadence,
#                   name='actual cadence', line=dict(color='firebrick', width=2)))

#     fig.add_trace(go.Scatter(x=seconds, y=derived_cadence,
#                   name='derived cadence', line=dict(color='royalblue', width=4)))
#     # TODO remove once data is smoothed
